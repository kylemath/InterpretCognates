...
We supplement these six experiments with two validation analyses that assess the robustness of our embedding corrections and the role of surface-form overlap.
...
To disentangle orthographic from semantic contributions to embedding convergence, we compute the mean normalized Levenshtein similarity of each concept's word forms across Latin-script languages in our corpus and regress convergence scores against this orthographic predictor.
The resulting $R^2$ quantifies the fraction of convergence variance attributable to surface-form overlap---the signature of loanword-driven rather than semantically-driven convergence.
