...
\begin{figure*}[htbp]
  ...
  \caption{Swadesh convergence ranking. Each concept is plotted by mean cross-lingual orthographic similarity (normalized Levenshtein distance on raw word forms) among Latin-script translations (x-axis) versus embedding convergence in NLLB-200's encoder (y-axis, isotropy-corrected). Points are colored by semantic category. The regression line ($R^2 = \DecompRsqOrtho{}$) confirms that surface-form similarity does not predict convergence. Concepts in the upper-left region converge strongly in embedding space \emph{despite} low surface-form similarity---the strongest candidates for genuine conceptual universals.}
  \label{fig:swadesh}
\end{figure*}
...
Crucially, Figure~\ref{fig:swadesh} demonstrates that surface-form similarity does not drive Swadesh convergence: regressing convergence scores against mean orthographic similarity (normalized Levenshtein distance on raw word forms across Latin-script languages) yields $R^2 = \DecompRsqOrtho{}$, and a parallel regression against mean phonological similarity explains only $R^2 = \DecompRsqPhon{}$.
Over 98\% of the convergence signal is thus attributable to semantic rather than surface-form factors, confirming that the convergence of Swadesh items reflects deeper conceptual structure.
...
\begin{figure}[htbp]
  ...
  \caption{Phylogenetic structure in the NLLB-200 embedding space. Left: heatmap of pairwise embedding similarity (normalized to $[0,1]$, where $0$ = no similarity and $1$ = identical) between \MantelNumLangs{} languages, ordered by hierarchical clustering. Right: dendrogram derived from the embedding distance matrix. Major language families (e.g., Indo-European, Austronesian, Niger-Congo) form recognizable clusters.}
  \label{fig:phylo}
\end{figure}
