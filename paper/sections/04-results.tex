...
To quantify the orthographic contribution to convergence, we regress each concept's corrected convergence score against its mean orthographic similarity (normalized Levenshtein similarity across Latin-script language pairs).
The orthographic predictor explains $R^2 = \DecompRsqOrtho{}$ of the variance in convergence scores (slope $= \DecompSlopeOrtho{}$), confirming that surface-form overlap accounts for only a small fraction of the embedding convergence signal.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_variance_decomposition.pdf}
  \caption{Variance decomposition: corrected convergence score vs.\ mean orthographic similarity (Levenshtein) for each Swadesh concept, colored by semantic category. The weak regression ($R^2 = \DecompRsqOrtho{}$) confirms that surface-form overlap explains little of the convergence signal.}
  \label{fig:variance_decomp}
\end{figure}

The result shown in Figure~\ref{fig:variance_decomp} is reassuring: concepts with high convergence scores are not simply those with cognate or borrowed word forms.
The residual convergence after controlling for orthographic similarity reflects the semantic component that our subsequent experiments probe.
