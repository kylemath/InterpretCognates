...
Our isotropy validation (Section~4) confirms that raw cosine similarity in the NLLB-200 encoder space is inflated and poorly calibrated.
...
The non-Swadesh comparison vocabulary is heavily skewed toward loanwords of Greek, Latin, or European origin (e.g., \textit{telephone}, \textit{democracy}, \textit{university}, \textit{hotel}), which share surface forms across many languages by virtue of cultural borrowing rather than independent coinage.
This selection bias, rather than any property of cultural stability, explains the higher convergence observed for the non-Swadesh set.
A fairer test of the cultural-stability hypothesis would require a control set matched for borrowability and cross-linguistic orthographic diversity.
Additionally, translations for the non-Swadesh vocabulary were generated by a language model rather than verified by native speakers, introducing further noise.
...
The per-family offset consistency patterns observed in Figure~\ref{fig:offset_family_heatmap} suggest that language-family information is encoded in specific subspaces; attention-head analysis could localize this encoding.
